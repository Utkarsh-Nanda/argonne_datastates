...
bool InsertOperation(KVStore& store, const std::string& key, const std::vector<Field>& values) {
    ...
    return store.insert(key, serialized_data);
}
...
bool UpdateOperation(KVStore& store, const std::string& key, const std::vector<Field>& new_values) {
    ...
    return store.insert(key, updated_data);
}
...
int main() {
	...
	//const int num_threads = 8;
        ...
        std::cout << "\nEphemeral store benchmarks:\n";
        double ephemeral_insert_diff = measure_throughput(*ephemeral_store, false, num_operations, num_threads, 0,
                                                          field_count, field_prefix, field_length, ordered_inserts, zero_padding);
        double ephemeral_insert_same = measure_throughput(*ephemeral_store, true, num_operations, num_threads, initial_load,
                                                          field_count, field_prefix, field_length, ordered_inserts, zero_padding);
        ...
        std::cout << "\nEphemeral store throughput (ops/sec):\n";
        std::cout << "  Insert (different keys): " << ephemeral_insert_diff << "\n";
        std::cout << "  Insert/Update (same keys): " << ephemeral_insert_same << "\n";
