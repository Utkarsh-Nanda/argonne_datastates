...
// // Constants
// int N = 100000;  // Number of key-value pairs
...
//     std::cout << "Starting insertion with " << t << " threads..." << std::endl;
...
//     std::cout << "Starting updates with " << t << " threads..." << std::endl;
//     auto start = std::chrono::high_resolution_clock::now();
...
//     #pragma omp parallel num_threads(t)
//     {
//         // std::mt19937 rng(43 + omp_get_thread_num());
//         #pragma omp for
//         for (int i = 0; i < n; i++) {
...
// template<typename Map>
// void run_read(Map& vmap, const int n, const int t) {
...
//     #pragma omp parallel num_threads(t)
//     {
//         std::mt19937 rng(44 + omp_get_thread_num());
//         #pragma omp for
//         for (int i = 0; i < n; i++) {
//             if (keys.empty()) continue;
...
//             // Randomly select a version to read
//             int max_version = key_history_counts[key];
//             unsigned int read_version = rng() % max_version;
            
//             auto value = vmap.find(read_version, key);
...
//     auto end = std::chrono::high_resolution_clock::now();
//     auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
//     std::cout << "Reads completed in " << duration.count() << "ms" << std::endl;
//     std::cout << "Successful reads: " << successful_reads << "/" << n << std::endl;
// }
...
//     rocksdb_wrapper_t<std::string, std::string> vmap(DB_PATH);
